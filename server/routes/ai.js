...
router.post('/tips', async (req, res) => {
  try {
    ...
    const aiTip = await geminiService.generatePersonalizedTip(child, context || '');
...
router.post('/insights', async (req, res) => {
  try {
    ...
    const healthRecords = dataManager.findByChildId('healthRecords', childId);
    const aiInsight = await geminiService.generateHealthInsight(child, healthRecords);
...
router.post('/care-plan', async (req, res) => {
  try {
    ...
    const carePlan = await geminiService.generateCarePlan(child, specificNeeds || '');
...
router.get('/insights/:childId', (req, res) => {
  try {
    const insights = dataManager.findByChildId('aiInsights', req.params.childId);
    res.json(insights);
  } catch (error) {
...
router.delete('/insights/:id', (req, res) => {
  try {
    const success = dataManager.delete('aiInsights', req.params.id);
    if (!success) {
...
router.post('/daily-summary', async (req, res) => {
  try {
    ...
    const healthRecords = dataManager.findByChildId('healthRecords', childId);
    const reminders = dataManager.findByChildId('reminders', childId);
    const carePlans = dataManager.findByChildId('carePlans', childId);

    const prompt = `
      Generate a daily summary for a parent about their child's health and care needs.
      
      Child: ${child.name}, Age: ${geminiService.calculateAge(child.dateOfBirth)} years
